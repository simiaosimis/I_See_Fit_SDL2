...
void TileMap::load(const std::string& mapPath_) {
	...
	if(!this->map->HasError()) {
					...
					if(currentLayer->GetName() == "Collision") {
						if(this->tileMatrix[j][k][i] > 0) {
							...
							const std::string property =
								tile->GetProperties().GetList().begin()->second;

							SDL_Rect tileRect = {(int)(j * TILE_SIZE), (int)(k * TILE_SIZE),
								TILE_SIZE, TILE_SIZE};

							/// @todo Add collision tile to a collision vector?
						}
					}
				}
			}
		...
		Log(DEBUG) << "TileMap::load Map loaded (width:" << this->mapWidth << " height:" <<
			this->mapHeight << " layers:" << this->layers << ")";
...
void TileMap::renderLayer(const double cameraX_, const double cameraY_,
	...
	for (int x = 0; x < tilesInX; x++) {
		for (int y = 0; y < tilesInY; y++) {
			...
			if(tileIsOnScreen) {
				...
				tilePosition &= ~(Tmx::FlippedDiagonallyFlag | Tmx::FlippedHorizontallyFlag
					| Tmx::FlippedVerticallyFlag);
